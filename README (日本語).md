...
- 出力フォルダーを暗号化された保管庫として設定
- アプリ内で保管庫の ZIP を開き、エクスポート、削除
- 結果を ZIP ファイルとしてダウンロード
- AES-256 パスワードで ZIP を保護
- AES-256-GCM と Argon2id で保管庫の内容を保護
- 多言語インターフェースと初回起動ガイド
- ライトテーマとダークテーマ
- 調整可能な背景
- `F11` によるボーダーレス全画面表示
...
FCP は、変換と文字起こしをローカルマシンで直接実行するデスクトップアプリです。バッチ変換、動画のトランスコード、Whisper の文字起こしなどの重い処理では、CPU、メモリ、そして一時ディスク容量を多く消費します。
...
- 8 GB RAM
- 4 コア / 8 スレッド以上の CPU
- 一時ファイルと変換出力用に十分な空き容量のある SSD または HDD
- 専用 GPU は任意ですが、より滑らかな動画変換には推奨

### 推奨要件

- Windows 11（64 ビット）
- 16 GB RAM 以上
- 8 コア / 16 スレッド以上の CPU（例: 近年の Ryzen 7/9、Intel i7/i9）
- 一時ファイル処理と出力の高速化のため NVMe SSD を推奨
- 専用 GPU は動画処理の高速化と全体的な応答性向上のため推奨

> 注: 変換中に CPU 使用率が高くなるのは正常です。FCP はローカルでネイティブ FFmpeg と同梱の Whisper エンジンを使用するため、大きなメディアファイルや長い音声の文字起こしを処理する際には、最新のマルチコア CPU を十分に活用できます。
...
設定で出力フォルダーを選択し、「出力フォルダーを保管庫として設定」を有効にします。10 文字以上のパスワードと容量上限を設定してください。ZIP のエクスポート時には、通常の出力フォルダーまたは保管庫を選択できます。

保管庫は ZIP ファイルを、非表示の `.fcp-vault` フォルダー内に暗号化された `.fcpv` コンテナーとして保存します。内容と新しい ZIP ファイル名は AES-256-GCM で保護され、Argon2id がパスワードから暗号鍵を導出します。Windows エクスプローラーではコンテナーを確認できますが、内容の読み取りや復号はできません。設定の「保管庫を開く」からファイルの一覧表示、エクスポート、削除を行えます。

パスワードは保存されません。パスワードを失うと暗号化ファイルを復元できません。出力フォルダーを削除すると保管庫の設定は無効になりますが、既存データは削除されません。容量上限は保管庫の内容を制限しますが、Windows エクスプローラーでディスク領域を予約するものではありません。
